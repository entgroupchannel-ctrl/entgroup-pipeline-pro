...
import {
  ...
  Loader2, Trophy, Target, CheckCircle2, ChevronLeft, ChevronRight,
  LayoutGrid, User, AlertCircle, Edit2,
} from "lucide-react";
...
interface KpiActual {
  calls: number; emails: number; lines: number;
  meetings: number; quotations: number; revenue: number;
}
...
function pct(a: number, t: number) { return t > 0 ? Math.min(100, Math.round((a/t)*100)) : 0; }
  ...
  return (
    <svg width={size} height={size} className="rotate-[-90deg]">
      ...
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={color} strokeWidth={6}
        ...
        strokeDashoffset={circ * (1-p)} style={{ transition: "stroke-dashoffset 0.6s ease" }} />
    </svg>
  );
}
...
function MetricCard({ label, actual, target, icon, color, unit="" }: {
  ...
  return (
    <div className={`relative rounded-xl border bg-card p-4 transition-all ${done ? "border-emerald-300 dark:border-emerald-800" : ""}`}>
      {done && <CheckCircle2 className="absolute right-3 top-3 h-4 w-4 text-emerald-500" />}
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">{icon}{label}</div>
          <div className="mt-2 flex items-end gap-1">
            <span className="text-2xl font-bold tabular-nums">{actual.toLocaleString()}</span>
            {target > 0 && <span className="mb-0.5 text-sm text-muted-foreground">/ {target.toLocaleString()}{unit}</span>}
          </div>
          {target > 0 && (
          ...
          <div className="mt-1 text-[11px] text-muted-foreground">
            {target === 0 ? "ยังไม่มีเป้า" : done ? "✓ บรรลุเป้า" : `${p}% ของเป้า`}
          </div>
        </div>
...
// ── SalesKpiPanel — spotlight view for one person ─────────────────────────────

function SalesKpiPanel({ profile, target, actual, onSetTarget }: {
  profile: any;
  ...
  const overallScore = useMemo(() => {
    const scores = [
      target?.calls_per_day    ? pct(actual.calls,    target.calls_per_day)    : null,
      target?.emails_per_day   ? pct(actual.emails,   target.emails_per_day)   : null,
      target?.lines_per_day    ? pct(actual.lines,    target.lines_per_day)    : null,
      target?.meetings_per_week ? pct(actual.meetings, target.meetings_per_week) : null,
      target?.quotations_per_month ? pct(actual.quotations, target.quotations_per_month) : null,
      target?.revenue_target   ? pct(actual.revenue,  target.revenue_target)   : null,
    ].filter((s): s is number => s !== null);
    return scores.length ? Math.round(scores.reduce((a,b)=>a+b,0)/scores.length) : null;
  }, [target, actual]);

  const scoreColor = overallScore == null ? "text-muted-foreground"
    : overallScore >= 100 ? "text-emerald-600"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between rounded-xl border bg-card p-4">
        <div className="flex items-center gap-3">
          <Avatar className="h-12 w-12">
            <AvatarFallback className="bg-primary/10 text-lg font-bold text-primary">{ini}</AvatarFallback>
          </Avatar>
          <div>
          ...
          {overallScore != null && (
            <div className={`ml-2 text-3xl font-bold tabular-nums ${scoreColor}`}>
              {overallScore}%
              <span className="ml-1 text-xs font-normal text-muted-foreground">score วันนี้</span>
            </div>
          )}
        </div>
      ...
      {/* No target warning */}
      {!target && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-200">
          <AlertCircle className="h-4 w-4 shrink-0" />
          ยังไม่ได้ตั้งเป้าหมายให้พนักงานคนนี้
          <Button variant="outline" size="sm" className="ml-auto h-7 text-xs" onClick={onSetTarget}>
            ตั้งเลย
          </Button>
        </div>
      )}
      ...
      <div>
        <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          กิจกรรมวันนี้ / สัปดาห์นี้
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          <MetricCard label="โทรวันนี้"       actual={actual.calls}     target={target?.calls_per_day ?? 0}
            icon={<Phone className="h-3.5 w-3.5" />}         color="#0096C7" />
          <MetricCard label="Email วันนี้"    actual={actual.emails}    target={target?.emails_per_day ?? 0}
            icon={<Mail className="h-3.5 w-3.5" />}          color="#7c3aed" />
          <MetricCard label="Line วันนี้"     actual={actual.lines}     target={target?.lines_per_day ?? 0}
            icon={<MessageCircle className="h-3.5 w-3.5" />} color="#16a34a" />
          <MetricCard label="เข้าพบสัปดาห์นี้" actual={actual.meetings} target={target?.meetings_per_week ?? 0}
            icon={<Users className="h-3.5 w-3.5" />}         color="#d97706" />
          <MetricCard label="QT เดือนนี้"     actual={actual.quotations} target={target?.quotations_per_month ?? 0}
            icon={<FileText className="h-3.5 w-3.5" />}      color="#db2777" />
          <MetricCard label="Revenue เดือนนี้" actual={actual.revenue}  target={target?.revenue_target ?? 0}
            icon={<TrendingUp className="h-3.5 w-3.5" />}    color="#059669" unit="฿" />
        </div>
      </div>

      {/* Monthly KPI for this person */}
      <div className="rounded-xl border bg-card p-5">
        <MonthlyKpiSection focusUserId={profile.id} />
      </div>
    </div>
  );
}
...
function KpiPage() {
  ...
  // Manager view mode: "overview" = leaderboard, "spotlight" = drill-down one person
  const [viewMode,        setViewMode]        = useState<"overview" | "spotlight">("overview");
  ...
  const load = async () => {
    ...
    const [profRes, targRes, actRes, leadsRes] = await Promise.all([
      ...
      crmDb().from("activities").select("*")
        .gte("created_at", startOfMonth()).in("type", ["call","email","line","meeting"]),
  ...
  const actualsMap = useMemo(() => {
    ...
    const today = startOfDay(); const week = startOfWeek();
    for (const p of profiles) {
      const uid = p.id;
      const myActs  = activities.filter((a: any) => a.owner_id === uid);
      const myLeads = leads.filter((l: any) => l.owner_id === uid);
      map.set(uid, {
        calls:      myActs.filter((a: any) => a.type==="call"    && a.done && a.done_at >= today).length,
        emails:     myActs.filter((a: any) => a.type==="email"   && a.done && a.done_at >= today).length,
        lines:      myActs.filter((a: any) => a.type==="line"    && a.done && a.done_at >= today).length,
        meetings:   myActs.filter((a: any) => a.type==="meeting" && a.done && a.done_at >= week).length,
        quotations: myLeads.filter((l: any) => l.flowaccount_quotation_no || l.fa_inbound_id).length,
        revenue:    myLeads.filter((l: any) => l.stage==="won").reduce((s: number, l: any) => s + Number(l.expected_value ?? 0), 0),
      });
    }
    return map;
  ...
  const spotlightIdx = spotlightUserId ? salesList.findIndex(p => p.id === spotlightUserId) : -1;
  const prevSales = () => {
    const idx = spotlightIdx > 0 ? spotlightIdx - 1 : salesList.length - 1;
    setSpotlightUserId(salesList[idx]?.id ?? null);
  };
  const nextSales = () => {
    const idx = spotlightIdx < salesList.length - 1 ? spotlightIdx + 1 : 0;
    setSpotlightUserId(salesList[idx]?.id ?? null);
  };

  const openSpotlight = (uid: string) => {
    setSpotlightUserId(uid);
  ...
  const spotlightProfile = spotlightUserId ? salesList.find(p => p.id === spotlightUserId) : null;
  const spotlightTarget  = spotlightUserId ? targets.find(t => t.user_id === spotlightUserId) : undefined;
  const spotlightActual  = spotlightUserId ? (actualsMap.get(spotlightUserId) ?? { calls:0, emails:0, lines:0, meetings:0, quotations:0, revenue:0 }) : null;

  return (
    <div className="p-6 page-fade-in space-y-6">
      ...
      {isManager && (
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex rounded-lg border bg-muted/30 p-0.5 gap-0.5">
            ...
            >
              <LayoutGrid className="h-3.5 w-3.5" /> ภาพรวมทีม
            </button>
            <button
              onClick={() => {
                setViewMode("spotlight");
                if (!spotlightUserId && salesList.length > 0) setSpotlightUserId(salesList[0].id);
              }}
              className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${
                viewMode === "spotlight" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              <User className="h-3.5 w-3.5" /> ดูรายคน
            </button>
          </div>
          ...
          {viewMode === "spotlight" && (
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={prevSales}>
                <ChevronLeft className="h-4 w-4" />
              </Button>

              {/* Avatars pill list */}
              <div className="flex items-center gap-1.5 rounded-xl border bg-card px-3 py-1.5">
                {salesList.map((p) => {
                  const ini = initials(p.full_name ?? "?");
                  const isActive = p.id === spotlightUserId;
                  return (
                    <button
                      key={p.id}
                      onClick={() => setSpotlightUserId(p.id)}
                      title={p.full_name}
                      className={`relative flex h-8 w-8 items-center justify-center rounded-full text-[11px] font-semibold transition-all ${
                        isActive
                          ? "ring-2 ring-primary ring-offset-1 bg-primary text-primary-foreground scale-110"
              ...
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={nextSales}>
                <ChevronRight className="h-4 w-4" />
              </Button>

              <span className="text-xs text-muted-foreground">
                {spotlightIdx + 1} / {salesList.length}
              </span>
            </div>
          )}
        </div>
      ...
      {/* ── SPOTLIGHT MODE ── */}
      {isManager && viewMode === "spotlight" && spotlightProfile && spotlightActual && (
        <SalesKpiPanel
          profile={spotlightProfile}
          target={spotlightTarget}
          actual={spotlightActual}
          onSetTarget={() => { setTargetUserId(spotlightProfile.id); setSetTargetOpen(true); }}
        />
      )}

      {/* ── OVERVIEW MODE ── */}
      {(!isManager || viewMode === "overview") && (
        <>
          {/* My KPI */}
          <section>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              <MetricCard label="โทรวันนี้"       actual={myActual.calls}      target={myTarget?.calls_per_day ?? 0}        icon={<Phone className="h-3.5 w-3.5" />}         color="#0096C7" />
              <MetricCard label="Email วันนี้"    actual={myActual.emails}     target={myTarget?.emails_per_day ?? 0}       icon={<Mail className="h-3.5 w-3.5" />}          color="#7c3aed" />
              <MetricCard label="Line วันนี้"     actual={myActual.lines}      target={myTarget?.lines_per_day ?? 0}        icon={<MessageCircle className="h-3.5 w-3.5" />} color="#16a34a" />
              <MetricCard label="เข้าพบสัปดาห์นี้" actual={myActual.meetings}  target={myTarget?.meetings_per_week ?? 0}    icon={<Users className="h-3.5 w-3.5" />}         color="#d97706" />
              <MetricCard label="QT เดือนนี้"     actual={myActual.quotations} target={myTarget?.quotations_per_month ?? 0} icon={<FileText className="h-3.5 w-3.5" />}      color="#db2777" />
              <MetricCard label="Revenue เดือนนี้" actual={myActual.revenue}   target={myTarget?.revenue_target ?? 0}       icon={<TrendingUp className="h-3.5 w-3.5" />}    color="#059669" unit="฿" />
            </div>
            {!myTarget && (
                    ...
                    <tbody className="divide-y">
                      {profiles
                        .map((p) => {
                          ...
                          const scores = [
                            t?.calls_per_day    ? pct(actual.calls,    t.calls_per_day)    : null,
                            t?.emails_per_day   ? pct(actual.emails,   t.emails_per_day)   : null,
                            t?.lines_per_day    ? pct(actual.lines,    t.lines_per_day)    : null,
                            t?.meetings_per_week ? pct(actual.meetings, t.meetings_per_week) : null,
                            t?.quotations_per_month ? pct(actual.quotations, t.quotations_per_month) : null,
                          ].filter((s): s is number => s !== null);
                        ...
                        .map((p, idx) => {
                          ...
                          return (
                            <tr
                              key={p.id}
                              className={`hover:bg-muted/30 transition-colors cursor-pointer ${p.id === user?.id ? "bg-primary/5" : ""}`}
                            ...
                            >
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  <Avatar className="h-7 w-7">
                                    <AvatarFallback className="bg-primary/10 text-[10px] text-primary">{ini}</AvatarFallback>
                                  </Avatar>
                                  <div>
                                    <div className="font-medium text-sm text-primary hover:underline">{p.full_name ?? "—"}</div>
                              ...
                              <td className="px-4 py-3 text-right text-sm font-medium">
                                ...
                                {t?.revenue_target ? <div className="text-[10px] text-muted-foreground">/ {formatBaht(t.revenue_target)}</div> : null}
                              </td>
...
function SetTargetDialog({ open, onOpenChange, userId, profiles, existing, onSaved }: {
  open: boolean; onOpenChange: (v: boolean) => void;
  userId: string; profiles: any[];
  existing: KpiTarget | null; onSaved: () => void;
}) {
  const { user } = useAuth();
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({
    calls_per_day: "0", emails_per_day: "0", lines_per_day: "0",
    meetings_per_week: "0", quotations_per_month: "0", pos_per_month: "0", revenue_target: "0",
  });
  ...
  useEffect(() => {
    ...
    } : { calls_per_day:"0", emails_per_day:"0", lines_per_day:"0", meetings_per_week:"0", quotations_per_month:"0", pos_per_month:"0", revenue_target:"0" });
  }, [open, existing]);
        ...
        <div className="space-y-3 pt-1">
          {fields.map(({ key, label, icon, unit }) => (
            <div key={key} className="flex items-center gap-3">
              <div className="flex w-44 items-center gap-1.5 text-xs text-muted-foreground shrink-0">{icon}{label}</div>
